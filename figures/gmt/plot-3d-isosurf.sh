...
gmt set MAP_FRAME_WIDTH         = 0.75p
gmt set MAP_TICK_PEN_PRIMARY    = 0.75p
gmt set MAP_GRID_PEN_PRIMARY    = 0.75p,black,_
...
INPUTFILE=${CVM}-iso-vs${TARGET}-s250m.txt
...
gmt psbasemap -JX5i/3.7i -Y3i -Bwesn -R0/180000/0/135000 -p$PERSP/0 -P -K > $OUTFILE

# plot the surface with transparent facades and contour lines
gmt grdview $THEGRID -C$THECPT -R0/180000/0/135000/0/$CMAX -S0.25 -J -JZ-1.5i -p$PERSP/0 -Wm0.25p,200 -N${CMAX}/200 -Bz2000 -Qs -Wc -Wm -V -K -O >> $OUTFILE
...
# shore
# gmt psxy $SHORE -R -JX -JZ -W0.75p -G30/120/190@50 -p -K -O >> $OUTFILE
gmt psxy $SHORE -R -JX -JZ -W0.75p -p -K -O >> $OUTFILE
...
# plot color bar
gmt set MAP_TICK_LENGTH_PRIMARY = 7p/3p
